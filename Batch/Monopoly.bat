...
:: NOTE - I did not create this program. I am merely uploading it here for reference because
::        I did not see it on GitHub to fork, do not know the creator, and I enjoy the code.
...
if "%DiceOne%" EQU "0" goto :RollADice
if "%DiceOne%" GTR "6" goto :RollADice
if "%DiceTwo%" EQU "0" goto :RollADice
if "%DiceTwo%" GTR "6" goto :RollADice
...
echo.
 :ChooseCommunityCard
:ChooseCommunityCard
set /a ChooseCommunityCard=%random:~0,2% + 1
...
echo.
 :ChooseChanceCard
:ChooseChanceCard
set /a ChooseChanceCard=%random:~0,2% + 1
if not "%ChooseChanceCard:~1,1%" == "" if "%ChooseChanceCard:~0,1%" GTR "1" (goto :ChooseChanceCard) ELSE (if "%ChooseChanceCard:~1,1%" GTR "6" goto :ChooseChanceCard)
if "!FreeOutOfJail_Chance!" == "1" if "%ChooseChanceCard%" == "12" goto :ChooseChanceCard
if "%ChooseChanceCard%" == "16" set ChooseChanceCard=7
...
echo ³  Chance Card   ³
echo ³                ³
FOR /F "tokens=1-15 delims=/" %%A IN ("³ Your building  ³/³ Bank pays you  ³/³    Go back     ³/³  Make general  ³/³ Take a walk on ³/³Advance token to³/³Advance token to³/³   Advance to   ³/³  Pay poor tax  ³/³ Advance to Go. ³/³ Advance to St. ³/³Get out of Jail ³/³ Take a ride on ³/³ Go directly to ³/³ You have been  ³/") DO echo %Var%
FOR /F "tokens=1-15 delims=/" %%A IN ("³    and loan    ³/³  dividend of   ³/³   3 spaces.    ³/³ repairs on all ³/³ the Boardwalk. ³/³nearest utility.³/³  the nearest   ³/³Illinois Avenue.³/³     of $15.    ³/³                ³/³ Charles Place. ³/³     free.      ³/³  the Reading   ³/³  Jail, do not  ³/³elected Chairman³/") DO echo %Var%
